...
const port = new SerialPort('/dev/cu.usbmodem1400', {
    baudRate: 9600
});
...
parser.on('data', function(buffer) {
    var offset = 0;
    var code = buffer.readUInt16LE(offset);
    offset += 2;
    console.log(code);
    ...
    switch (code) {
        case 1: // Packet of hedgehog coordinates with cm resolution coordinates
            offset += 1;
            data = bufferpack.unpack('<LHHHBBH', buffer, offset);
            offset += 14;
        ...
        case 17: // Packet of hedgehog coordinates with mm resolution coordinates
            {
                ...
                //(address, locationX, locationY, timestamp)
                setRobotParams(data[5], data[1], data[2], data[0]);

                break;
            }
        case 2: // Packet of all beacons coordinates with cm resolution coordinates
            offset += 1;
            nbeacons = nbytes = buffer.readUInt8(offset);
            offset += 1;
            beaconsData = [];
            for (var i = 0; i < nbeacons; i++) {
                beaconData = bufferpack.unpack('<BHHH', buffer, offset);
                offset += 7;
                offset += 1;
                beaconData = {
        ...
        case 18: // Packet of all beacons coordinates with mm resolution coordinates
            {
                ...
                beaconsData = [];
                for (var i = 0; i < nbeacons; i++) {
                    beaconData = bufferpack.unpack('<BLLL', buffer, offset);
                    offset += 13;
                    offset += 1;
                    beaconData = {
                        'address': beaconData[0],
                ...
                currentNumRobotsInPlace = nbeacons;
                break;
            }
    }
